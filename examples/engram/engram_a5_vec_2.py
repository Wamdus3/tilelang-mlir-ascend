    ...
    def engram_gate_fwd_kernel(
        ...
        with T.Kernel(hc_mult * num_persistent_blocks, is_npu=True) as (cid, _):
            ...
            for i_s in T.serial(t_start, t_end):
                ...
                x_vec = T.alloc_shared((hidden_size,), dtype)
                x_vec_cast = T.alloc_fragment((hidden_size,), accum_dtype)
                k_vec = T.alloc_shared((hidden_size,), dtype)
                k_vec_cast = T.alloc_fragment((hidden_size,), accum_dtype)
                ...
                T.vcast(x_vec, x_vec_cast)
                T.vcast(k_vec, k_vec_cast)

                T.vmul(x_vec_cast, x_vec_cast, tmp_vec)
                T.reduce_sum(tmp_vec, rstd_x_reducer, dim=0, clear=True)

                T.vmul(k_vec_cast, k_vec_cast, tmp_vec)
                T.copy(tmp_vec, kk_out[i_s, pid_h, :])
                T.reduce_sum(tmp_vec, rstd_k_reducer, dim=0, clear=True)
