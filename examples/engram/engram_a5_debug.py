...
def get_engram_gate_fwd_kernel(
    ...
    def engram_gate_fwd_kernel(
        ...
        with T.Kernel(hc_mult * num_persistent_blocks, is_npu=True) as (cid, _):
            ...
            kk_local = T.alloc_shared((vec_size,), accum_dtype)
            ...
            for i_s in T.serial(t_start, t_end):
                ...
                for i_b in T.serial(1, num_blk):
                    ...
                    for i_sub in T.serial(sub_blks):
                        ...
                        for tid in T.serial(threads):
                            ...
                            T.vmul(k_local_1, k_local_1, kk_local)
                            T.copy(
                                kk_local,
                                kk_out[i_s, pid_h, sub_base + tid * vec_size: sub_base + tid * vec_size + vec_size],
                            )
                            for i_k in T.serial(vec_size):
                                rstd_x_local[0] += x_local_1[i_k] * x_local_1[i_k]
                                rstd_k_local[0] += kk_local[i_k]
                                gate_score_local[0] += (
                ...
                for i_sub in T.serial(sub_blks):
                    ...
                    for tid in T.serial(threads):
                        ...
                        T.vmul(k_local_2, k_local_2, kk_local)
                        T.copy(
                            kk_local,
                            kk_out[i_s, pid_h, sub_base + tid * vec_size: sub_base + tid * vec_size + vec_size],
                        )
...
def run_test():
    ...
    print("raw_dot ref:")
    print(dot_ref)
    print("gate_score kernel:")
    print(gate_score)
    print("gate_score ref:")
    print(gate_score_ref)
