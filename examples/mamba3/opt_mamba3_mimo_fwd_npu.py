    ...
    def mamba_mimo_fwd_kernel(
        ...
        with T.Kernel(H * B, is_npu=True) as (idx, _):
            ...
            for i in T.Pipelined(0, nchunks, num_stages=num_stages):
                ...
                q_first_sin = T.alloc_fragment([fused_chunk_size, rotary_dim], accum_dtype)
                q_second_sin = T.alloc_fragment(
                    [fused_chunk_size, rotary_dim], accum_dtype
                )
                q_first_cos = T.alloc_fragment([fused_chunk_size, rotary_dim], accum_dtype)
                ...
                k_first_sin = T.alloc_fragment([fused_chunk_size, rotary_dim], accum_dtype)
                k_second_sin = T.alloc_fragment(
                    [fused_chunk_size, rotary_dim], accum_dtype
                )
                k_first_cos = T.alloc_fragment([fused_chunk_size, rotary_dim], accum_dtype)
                ...
                da_cs__or__exp_da_cs_shared = T.alloc_shared([chunk_size, 1, 1], "float32")
                T.copy(
                ...
                o_mimo_accum_frag_reshaped = T.alloc_fragment([chunk_size, R, P], accum_dtype)
                T.reshape(o_mimo_accum_frag, o_mimo_accum_frag_reshaped)
                T.copy(da_cs__or__exp_da_cs_shared, exp_da_cs_frag)

                T.vmul(o_mimo_accum_frag_reshaped, exp_da_cs_frag, o_mimo_accum_frag_reshaped)
...
FULL_SHAPE_CASES = [
    {
        "name": "upstream_smoke_readme_shape",
        "n": 128,
        "p": 64,
        "r": 4,
        "chunk_size": 16,
    ...
    {
        "name": "upstream_optimized_target",
        "n": 128,
        "p": 64,
        "r": 4,
        "chunk_size": 16,
...
def run_forward(inputs: dict) -> Tensor:
    out, _, _ = mamba_mimo_forward(
        inputs["q"],
        inputs["k"],
        inputs["v"],
        inputs["q_bias"],
...
def run_reference(inputs: dict, r: int) -> Tensor:
    orig_kron = torch.kron

    def kron_for_mimo_mask(a: Tensor, b: Tensor) -> Tensor:
        if (
            a.ndim == 5
            and b.ndim == 5
            and tuple(b.shape[:3]) == (1, 1, 1)
            and b.shape[-2] == r
            and b.shape[-1] == r
            and bool(torch.all(b == 1).item())
        ):
            return a.repeat_interleave(r, dim=-2).repeat_interleave(r, dim=-1)
        return orig_kron(a, b)

    torch.kron = kron_for_mimo_mask
    try:
        out, _, _ = mamba3_MIMO_chunk_ref(
            ...
            dtype=FIXED_DTYPE,
            return_final_state=False,
            contract_mimo_out=True,
        )
    finally:
        torch.kron = orig_kron
    return out
...
def run_case(case: dict) -> None:
    print(
        f"[case] {case['name']} "
        f"B={case['b']} S={case['s']} H={case['h']} G={case['g']} "
        f"N={case['n']} P={case['p']} R={case['r']} chunk={case['chunk_size']}",
        flush=True,
    )
    ...
    out_ref = run_reference(inputs, case["r"])
    torch.npu.synchronize()
    out_tilelang = run_forward(inputs)
    torch.npu.synchronize()
    ...
    assert_stable_rel(
        out_tilelang.float().cpu(),
        out_ref.float().cpu(),
        label="O",
        cfg=cfg,
    )
    print(f"[case] {case['name']} PASS", flush=True)

    del inputs, out_ref, out_tilelang
    gc.collect()
...
def run_test():
    ...
    for case in FULL_SHAPE_CASES:
        run_case(case)
    print("All full-shape tests passed!")
